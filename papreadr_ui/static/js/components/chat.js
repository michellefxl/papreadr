...
function setUserResponse(message) {
    const user_response = `<img class="userAvatar" src='./static/img/usr_icon.png'><p class="userMsg">${message} </p><div class="clearfix"></div>`;
...
function getBotResponse(text) {
    botResponse = `<img class="botAvatar" src="./static/img/prb_icon.png"/><span class="botMsg">${text}</span><div class="clearfix"></div>`;
    return botResponse;
}
...
function setBotResponse(response) {
    ...
    setTimeout(() => {
        ...
        if (response.length < 1) {
            ...
            const fallbackMsg = "🤔 Hmm...I don't have the answer to that.";

            const BotResponse = `<img class="botAvatar" src="./static/img/prb_icon.png"/><p class="botMsg">${fallbackMsg}</p><div class="clearfix"></div>`;
            ...
            for (let i = 0; i < response.length; i += 1) {
                ...
                if (Object.hasOwnProperty.call(response[i], "text")) {
                    if (response[i].text != null) {
                        ...
                        if (html.includes("<ul") || html.includes("<ol") || html.includes("<li") || html.includes('<h3')) {
                            ...
                            if (!botResponse) {
                                botResponse = `<img class="botAvatar" src="./static/img/prb_icon.png"/><p class="botMsg">${response[i].text}</p><div class="clearfix"></div>`;
                            }
                        }
                ...
                if (Object.hasOwnProperty.call(response[i], "image")) {
                    if (response[i].image !== null) {
                        const BotResponse = `<div class="singleCard"><a href="${response[i].image}" target="_blank"><img class="imgcard" src="${response[i].image}" width:auto; height:auto;></a></div><div class="clearfix">`;
